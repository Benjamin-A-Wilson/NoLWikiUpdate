...
function AssignWork(&$gradebook, $auth){
    ...
    foreach($gradebookarticles as $k => $v){
        ...
        $author = str_replace('_','',strtolower(PageVar($pagename,'$:Author')));
...
function ExportGrades($pagename, $auth) {
    ...
    // Open CSV file
    $write = fopen("./local/grades.csv","w");
    ...
    foreach ($profiles as $v) {
        ...
        foreach ($statuses as $j) {
            foreach ($items as $k) {
                ...
                DPrint("Value for " . $name . " at key " . $name . " is " . $gradebook[$name][$key][0] . " | " . $gradebook[$name][$key][1],4);
    ...
    $write_success = fwrite($write,$exportstring);
    DPrint("Write to file: " . $write_success,1);
